...
class Header {
  ...
  validate () {
    ...
    if (_.size(missingHeaders) > 0)
      for (const missingHeader of Array.from(missingHeaders))
  ...
  _languageToIndex (localizedAttributes) {
    const langH2i = {}
    for (const langAttribName of Array.from(localizedAttributes))
  ...
  _productTypeLanguageIndexes (productType) {
    ...
    if (!langH2i) {
      const ptLanguageAttributes = _.map(productType.attributes, (attribute) => {
  ...
  missingHeaderForProductType (productType) {
    this.toIndex()
    return _.filter(productType.attributes, attribute => !this.has(attribute.name) && !this.productTypeAttributeToIndex(productType, attribute))
  }
}
