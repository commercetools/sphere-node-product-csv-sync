...
class Export {
  ...
  constructor(options) {
    ...
    this.queryOptions = {
      ...
      filterVariantsByAttributes: this._parseQuery(
        this.options.export != null ? this.options.export.filterVariantsByAttributes
       : undefined),
  ...
  _filterVariantsByAttributes(variants, filter) {
    const filteredVariants = _.filter(variants, function(variant) {
      if ((filter != null ? filter.length : undefined) > 0) {
        return _.reduce(
          filter,
          function(filterOutVariant, filter) {
            // filter attributes
  ...
  _fetchResources() {
    ...
    .then((...args) => {
      ...
      return Promise.resolve({productTypes, categories, channels, customerGroups, taxes});
    });
  }
  ...
  exportDefault(templateContent, outputFile, staged) {
    ...
    .then(({productTypes}) => {
      return this.export(templateContent, outputFile, productTypes, staged, false, true);
    });
  }
  ...
  exportFull(output, staged) {
    ...
    .then(({productTypes}) => {
      ...
      return Promise.map(productTypes.body.results, type => {
      ...
      if (_.size(errors) !== 0) {
        ...
        .then(function() {
          if (createFileWhenEmpty || rowsReaded) {
            ...
            return Promise.resolve();
          }}).then(() => Promise.resolve("Export done.")).catch(function(err) {
  ...
  _parse(csvString) {
    return new Promise((resolve, reject) => {
      ...
      .to.array(function(data, count) {
        ...
        return resolve(header);}).on('error', err => reject(err));
    });
  }
}
