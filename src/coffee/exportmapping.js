...
class ExportMapping {
  ...
  mapProduct(product, productTypes) {
    ...
    if (product.variants) {
      for (let variant of Array.from(product.variants)) {
  ...
  _mapBaseProduct(product, productType) {
    ...
    if (this.header.has(CONS.HEADER_CATEGORIES)) {
      row[this.header.toIndex(CONS.HEADER_CATEGORIES)] = _.reduce(product.categories || [], (memo, category, index) => {
  ...
  _mapVariant(variant, productType, row) {
    ...
    if (variant.attributes) {
      for (let attribute of Array.from(variant.attributes)) {
  ...
  _mapPrices(prices) {
    return _.reduce(prices, (acc, price, index) => {
  ...
  _mapSetOfLenum(attribute, productType, row) {
    ...
    const noneLangIndex = this.header.toIndex(attribute.name);
    if (noneLangIndex) {
      row[noneLangIndex] = _.reduce(attribute.value, function(memo, val, index) {
        if (index !== 0) { memo += GLOBALS.DELIM_MULTI_VALUE; }
        return memo + val.key;
      }
      , '');
    ...
    if (h2i) {
      for (var lang in h2i) {
        const index = h2i[lang];
        if (attribute.value) {
          row[index] = _.reduce(attribute.value, function(memo, val, index) {
            if (index !== 0) { memo += GLOBALS.DELIM_MULTI_VALUE; }
  ...
  _mapSetAttribute(attribute, attributeTypeDef) {
    switch (attributeTypeDef.elementType.name) {
      ...
      case CONS.ATTRIBUTE_TYPE_MONEY:
        return _.reduce(attribute.value, (memo, val, index) => {
          if (index !== 0) { memo += GLOBALS.DELIM_MULTI_VALUE; }
