...
// current workflow:
  ...
  importManager(file, isArchived) {
    ...
    , {concurrency: 1})
    .then(res => Promise.resolve(_.flatten(res))).catch(function(err) {
  ...
  mapVariantsBasedOnSKUs(existingProducts, products) {
    ...
    const [sku2index, sku2variantInfo] = Array.from(existingProducts.reduce((aggr, p, i) =>
      ([p.masterVariant].concat(p.variants)).reduce(function(...args) {
        const [s2i, s2v] = Array.from(args[0]), v = args[1], vi = args[2];
        s2i[v.sku] = i;
        s2v[v.sku] = {
  ...
  update(product, existingProduct, id2SameForAllAttributes, header, rowIndex, publish) {
    ...
    if (this.dryRun) {
      ...
      if (allUpdateRequests.actions.length) {
        ...
        .then(result => {
          return this.publishProduct(result.body, rowIndex)
  ...
  create(product, rowIndex, publish) {
    ...
    if (this.dryRun) {
      ...
      .then(result => {
        return this.publishProduct(result.body, rowIndex, true, publish)
