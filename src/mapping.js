...
class Mapping {
  ...
  mapProduct (raw, productType) {
    ...
    _.each(raw.variants, (entry, index) => product.variants.push(this.mapVariant(entry.variant, index + 2, productType, entry.rowIndex, product)))
  ...
  mapBaseProduct (rawMaster, productType, rowIndex) {
    ...
    for (const attribName of Array.from(CONS.BASE_LOCALIZED_HEADERS)) {
      var val
  ...
  mapCategories (rawMaster, rowIndex) {
    ...
    for (const rawCategory of Array.from(rawCategories)) {
      var msg
      const cat =
        { typeId: 'category' }
    ...
    if (elementType.name === CONS.ATTRIBUTE_TYPE_LTEXT) {
      ...
      _.each(multiValObj, (raw, lang) => {
        if (this.isValidValue(raw)) {
          ...
          return _.each(languageVals, (v, index) => {
            const localized = {}
            localized[lang] = v
  ...
  mapPrices (raw, rowIndex) {
    ...
    for (const rawPrice of Array.from(rawPrices)) {
  ...
  mapImages (rawVariant, variantId, rowIndex) {
    ...
    for (const rawImage of Array.from(rawImages)) {
      const image = {
        url: rawImage,
