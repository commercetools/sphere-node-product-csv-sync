...
const readXlsx = function(filePath) {
  const values = [];
  ...
  .then(function() {
    ...
    worksheet.eachRow(function(row) {
      ...
      rowValues.shift();
      return values.push(rowValues);
    });
    return values;
  });
};

describe('Export xlsx integration tests', function() {
  ...
  it('should export to xlsx based on minimum template', function(done) {
    const outputLocation = '/tmp/output.xlsx';
    const template =
      `\
...
`;
    ...
    .then(function(result) {
      ...
      return readXlsx(outputLocation);}).then(function(rows) {
      expect(rows.length).toBe(expectedResult.length);
      _.map(rows, (row, index) => expect(row.toString()).toBe(expectedResult[index].toString()));
      return done();}).catch(err => done(_.prettify(err)));
  });

  it('should export to xlsx labels of lenum and set of lenum', function(done) {
    const outputLocation = '/tmp/output.xlsx';
    const template =
    `\
...
`;
  ...
  it('should do a full export to xlsx', function(done) {
    ...
    .then(function(result) {
      ...
      return readXlsx(xlsxFile);}).then(rows => {
      ...
      .then(function(res) {
        ...
        return expect(rows[1].toString()).toBe(expectedProductLine);
      });
  ...
  it('should export data to xlsx in utf-8 even when selected different encoding', function(done) {
...
`;
  ...
  it('should export product to xlsx with money set attribute', function(done) {
...
`;
    ...
    .then(() => {
      ...
      expect(result).toBe('Export done.');
      return readXlsx(outputLocation);}).then(rows => {
      expect(rows.length).toBe(expectedResult.length);
      _.map(rows, (row, index) => expect(row.toString()).toBe(expectedResult[index].toString()));
  ...
  it('should do a full xlsx export with queryString', function(done) {
  ...
  return it('should do a full xlsx export with encoded queryString', function(done) {
    const exporter = new Export({
      client: Config,
      exportFormat: "xlsx",
      export: {
