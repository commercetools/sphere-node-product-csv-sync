...
class MatchUtils {
  ...
  static mapBySku (product) {
    return [product.masterVariant].concat(product.variants || []).reduce(
      (agg, v) => _.extend(agg, Helpers.initMap(v.sku, product))
      , {},
    )
  }
  ...
  // @param {Object} product - Product
  static mapByCustomAttribute (attrName) {
    return product =>
      [product.masterVariant].concat(product.variants || []).reduce(
        (agg, v) => {
  ...
  static initMatcher (matchBy, existingProducts) {
    ...
    const map = existingProducts.reduce(
      (m, p) => _.extend(m, mapper(p))
      , {},
    )
